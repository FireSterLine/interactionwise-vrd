...
class DSRModel(nn.Module):
  def __init__(self, args):
    ...
    if self.args.feat_used.sem:
      self.emb = nn.Embedding(self.args.n_obj, self.args.emb_size)
      set_trainability(self.emb, requires_grad=False)
      self.total_fus_neurons += self.args.n_fus_neurons
      if self.args.feat_used.sem == "concat":
        self.fc_semantic = FC(self.args.emb_size*2, self.args.n_fus_neurons)
      ...
      # self.emb = nn.Embedding(self.n_obj, self.args.emb_size)
      # set_trainability(self.emb, requires_grad=False)
      # self.fc_so_emb = FC(self.args.emb_size*2, 256)
    ...
    if not self.args.use_pred_sem:
      ...
      if mode < 8:
        ...
        self.fc_fusion = FC(self.total_fus_neurons, self.args.emb_size, relu = False)
        if mode < 10:
