...
def parse_args():
  ...
  parser.add_argument('--image_dir', dest='image_dir',
                      ...
                      default="images")
  parser.add_argument('--image_outdir', dest='image_outdir',
                      help='directory where to store output images for demo',
                      default="images_det")
...
if __name__ == '__main__':
  ...
  if args.dataset == "pascal_voc":
    ...
    # with open('data/genome/150-50-50/objects_vocab.txt', 'r') as f:
    #with open('data/genome/2500-1000-500/objects_vocab.txt', 'r') as f:
    with open('data/genome/1600-400-20/objects_vocab.txt', 'r') as f:
      data = f.readlines()
    classes = np.asarray(['__background__'])
    #classes = np.asarray([])
    classes = np.append(classes, np.asarray(data))
    classes = [x.strip('\n') for x in classes]
  else:
  ...
  while (num_images >= 0):
      ...
      if vis and webcam_num == -1:
          ...
          # cv2.waitKey(0)
          result_path = os.path.join(args.image_outdir, imglist[num_images][:-4] + ".jpg")
