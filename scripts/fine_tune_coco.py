...
def fine_tune_embeddings_coco(path_to_model, tokenized_captions, num_epochs):
    ...
    vrd_embedder = VRDEmbedding(path_prefix, dim=dim, model=model_type)
    # model = vrd_embedder.load_model(os.path.join(path_to_model, "epoch_4.model"))
    model = vrd_embedder.fine_tune_model_coco(path_to_model, model_type, dim, tokenized_captions, num_epochs)
    return model
...
if __name__ == '__main__':
    ...
    if not os.path.exists(tokenized_captions_pickle) and not os.path.exists(tokenized_captions_txt):
        ...
        tokenized_captions = vrd_tokenize(lemma_captions, multi_word_phrases, multi_word_phrase_mappings,
                                          tokenize_regex)
